...
export default function SocialMedia() {
  return (
    <section className="py-20 px-6 lg:px-16 bg-[hsl(var(--dark-3))] text-center">
      <div className="section-label justify-center reveal">تابعنا على السوشيال ميديا</div>
      <h2 className="section-title reveal mb-12" style={{ transitionDelay: '100ms' }}>
        نحن في كل <span className="text-[hsl(var(--gold))]">مكان</span>
      </h2>

      <div className="flex justify-center gap-6 flex-wrap reveal" style={{ transitionDelay: '200ms' }}>
        {platforms.map((p, i) => (
          <a
            key={i}
            href={p.href}
            ...
            className="group flex flex-col items-center gap-4 px-10 py-9 bg-[hsl(var(--dark-4))] border border-[hsl(var(--gold)/0.15)] rounded-md text-decoration-none min-w-[180px] hover:-translate-y-2 transition-all duration-400"
            style={{
          ...
          >
            ...
            <div>
              ...
              <div className="text-sm opacity-50">{p.handle}</div>
            </div>
            <span className="text-xs tracking-wider" style={{ color: p.color }}>تابعنا &#8592;</span>
          </a>
        ))}
      </div>
    </section>
